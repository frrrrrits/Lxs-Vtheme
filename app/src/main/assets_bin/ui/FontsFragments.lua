...
require "import"
import "initapp"
import "ui.adapter.FontsAdapter"
...
local function newInstance()
  ...
  fontsAdapter = FontsAdapter(data,params,fragment,ids)

  local function setupRecycler()
    local manager = GridLayoutManager(this,2)
    ids.recycler.adapter=fontsAdapter
  ...
  function doFontsFragment(query)
    resetData()
    setupRecycler()
    ids.empty:hide()
        
    params.query = query
    ParseDatas.getData(data, 4, params, fontsAdapter, fragment, ids, true)
  end
